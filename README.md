...
- **0.1.0, 0.1.1, and 0.2.0 are published** with hosted `.smod` artifacts, recorded SHA-256 digests, and matching `releases/index.json` entries. 0.1.1 is the same declarative contract re-released as the upgrade target for the host upgrade path; 0.2.0 is a verification-milestone version marking the end-to-end device proof of the host flow and giving the host updater its first real upgrade target above 0.1.1. All are identical in declarative contract: on API 29 they install over one another preserving the module's enabled state, reject downgrades, and survive process restarts.
- **Host integration is implemented and device-verified** on the Convx fork (`modulehost` engine + Android seam: install, persist, enable/disable, remove, upgrade, check-for-updates against the live release feed) and shipped as upstream PRs #61, #62, and #63. It awaits upstream merge into a Convx release; until then, released Convx APKs do not yet expose the `.smod` host.
